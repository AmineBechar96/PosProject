                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <TransitionChild as="template" enter="ease-out duration-300"
                        ...
                        <DialogPanel
                            ...
                            <form @submit.prevent="form.patch(props.page_name.toLowerCase() + '/' + props.items.id, {
                                preserveScroll: true,
                                onStart: () => errors = null,
                                    ...
                                    <div class="sm:flex sm:items-start">
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            <DialogTitle as="h3"
                                                ...
                                                @form="form_method"></TextInput>
                                                <TextInput :form_inputs="{ input: 'Email',var_model:'email',var_edit:items.email }" :errors="errors.email"
                                                @form="form_method"></TextInput>
                                              <SelectInput v-if="page_name=='Waiters'"  title="Store" :data="stores" :var_edit="items.store_id" @form="form_method"></SelectInput>
                                              <TextAreaInput v-else-if="page_name=='Suppliers'" :form_inputs="{ input: 'Note' ,var_model:'note',var_edit:items.note}" :errors="errors.note"
                                            @form="form_method"></TextAreaInput>
                                              <TextInput v-else type="number" :form_inputs="{ input: 'Discount',var_model:'discount',var_edit:items.discount }" :errors="errors.discount"
                                                @form="form_method"></TextInput>                                     
                                        </div>
                                    </div>
                                </div>
