                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <TransitionChild as="template" enter="ease-out duration-300"
                        ...
                        <DialogPanel
                            ...
                            <form @submit.prevent="form.post(page_name.toLowerCase(), {
                                preserveScroll: true,
                                onStart: () => errors = null,
                                    ...
                                    <div class="sm:flex sm:items-start">
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            <DialogTitle as="h3"
                                                ...
                                                Add {{page_name}}</DialogTitle>
                                                <TextInput type="text" :form_inputs="{ input: 'Name',var_model:'name' }" :errors="errors.name"
                                                @form="form_method"></TextInput>
                                                <TextInput type="number" :form_inputs="{ input: 'Phone',var_model:'phone' }" :errors="errors.phone"
                                                @form="form_method"></TextInput>
                                                <TextInput type="email" :form_inputs="{ input: 'Email',var_model:'email' }" :errors="errors.email"
                                                @form="form_method"></TextInput>
                                                <SelectInput v-if="page_name=='Waiters'" title="Store" :data="stores" @form="form_method"></SelectInput> 
                                                <TextAreaInput v-else-if="page_name=='Suppliers'" :form_inputs="{ input: 'Note' ,var_model:'note'}" :errors="errors.note"
                                            @form="form_method"></TextAreaInput> 
                                                <TextInput v-else type="number" :form_inputs="{ input: 'Discount',var_model:'discount' }" :errors="errors.discount"
                                                @form="form_method"></TextInput>                                  
                                          </div>
                                    </div>
                                </div>
