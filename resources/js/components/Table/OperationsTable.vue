        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 cursor-pointer" @click="sort('phone')">
                        Phone 
                        <SortIcons :column="column" column_selected='phone' :direction="direction"></SortIcons>
                    </th>
                    <th scope="col" class="px-6 py-3 cursor-pointer" @click="sort('email')">
                        Email
                        <SortIcons :column="column" column_selected='email' :direction="direction"></SortIcons>
                    </th>
                    <th scope="col" class="px-6 py-3">
                        Action
                    </th>
                    <th scope="col" class="px-6 py-3 cursor-pointer" @click="sort('store')" v-if="routes=='Waiters'">
                        Store 
            ...
            <tbody>
                <tr v-for="item in data.data" :key="item.id"
                    ...
                    <td class="px-6 py-4" v-if="routes=='Waiters'">
                        {{ item.name2 }}
                    </td>
                    <td class="px-6 py-4">
...
function confirm_delete_item({ decision, id }) {
    ...
    if (decision == true) {
        router.delete("/" + props.routes.toLowerCase() + "/" + id, {
            preserveState: true,
            replace: true,
        });
    }
