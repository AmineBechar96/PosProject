...
function store_item(items) {
    activeAddModal.value = false;
    /*
    router.post("/categories", { items: items }, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
        onFinish: () => activeAddModal.value = false
    });*/
}
...
function update_item(items) {
    router.put('/categories' + items['id'], { items: items }, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
...
function filter({ column, direction, per_pages, search }) {
    ...
    router.get('/categories', { column: column_ref.value, direction: direction_ref.value, per_pages: per_page_ref.value, search: search_ref.value }, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
